...
class OCSMSCGateway {
	...
	'text' => $message
		...
		$jsonObj = json_decode($result);
		 if(null===$jsonObj) {
        $result_message = "Invalid JSON";
		}
		
		 elseif(!empty($jsonObj->error)) {
			// Ошибка отправки сообщения
			$result_message = "An error occured: " . $jsonObj->error . "(code: " . $jsonObj->code . ")";

			} else {
					// Сообщение успешно отправлено
					$result_message = "SMS message is sent. Message id " . $jsonObj->result->sms_id . ". SMS cost is " . $jsonObj->result->price . " " . $jsonObj->result->currency;

					}
					} else {
							// Ошибка соединения с API-сервером
							$result_message =  "API access error";
						}
		
		if ($this->config->get('oc_smsc_debug')) {
			$log = fopen(DIR_LOGS . 'smsc.log', 'a');
			fwrite($log, $result_message);
			fclose($log);
			}
