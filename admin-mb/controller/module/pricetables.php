...
class ControllerModulepricetables extends Controller {
	...
	public function index() {   
		$this->load->language('module/pricetables');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('pricetables', $this->request->post);		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/pricetables', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/pricetables', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		$modules = array();
		
		if (isset($this->request->post['pricetables_module'])) {
			$modules = $this->request->post['pricetables_module'];
		} elseif ($this->config->get('pricetables_module')) { 
			$modules = $this->config->get('pricetables_module');
		}
		...
		$this->template = 'module/pricetables.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		...
		if (!$this->user->hasPermission('modify', 'module/pricetables')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['pricetables_module'])) {
			foreach ($this->request->post['pricetables_module'] as $key => $value) {
				$module_number = $key;
