...
class ControllerModuleBlogCategory extends Controller {
	...
	public function index() {   
		...
		$this->data['action'] = $this->url->link('module/blog_category', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['blog_category_module'])) {
			$this->data['modules'] = $this->request->post['blog_category_module'];
		} elseif ($this->config->get('blog_category_module')) { 
			$this->data['modules'] = $this->config->get('blog_category_module');
		}	
		...
		$this->template = 'module/blog_category.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
