<?php
class ControllerModuleiconboxes extends Controller {
	private $error = array(); 
	 
	public function index() {   
		$this->load->language('module/iconboxes');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('iconboxes', $this->request->post);		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/iconboxes', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/iconboxes', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		$modules = array();
		
		if (isset($this->request->post['iconboxes_module'])) {
			$modules = $this->request->post['iconboxes_module'];
		} elseif ($this->config->get('iconboxes_module')) { 
			$modules = $this->config->get('iconboxes_module');
		}
		...
		$this->template = 'module/iconboxes.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		...
		if (!$this->user->hasPermission('modify', 'module/iconboxes')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['iconboxes_module'])) {
			foreach ($this->request->post['iconboxes_module'] as $key => $value) {
				$module_number = $key;
