...
class ControllerModuleCompleteSeo extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['mlseo_cache'])) {
			$this->data['mlseo_cache'] = (isset($this->request->post['mlseo_cache'])) ? true : false;
		} else {
			$this->data['mlseo_cache'] = $this->config->get('mlseo_cache');
		}
	...
	public function generator_product($mode, $simulate) {
		...
		{
			$this->config->set('seo_current_lang', $lang_code[$lang]);
			...
			foreach ($rows as $row)
			{
				...
				if($mode == 'url')
				{
					$value = $this->model_tool_seo_package->filter_seo($value, 'product', $row['product_id'], $lang);
	...
	public function generator_category($mode, $simulate) {
		...
		foreach($this->request->get['langs'] as $lang)
		{
			...
			foreach ($rows as $row)
			{
				...
				if($mode == 'url')
				{
					$value = $this->model_tool_seo_package->filter_seo($value, 'category', $row['category_id'], $lang);
	...
	public function generator_information($mode, $simulate) {
		...
		foreach($this->request->get['langs'] as $lang)
		{
			...
			foreach ($rows as $row)
			{
				...
				if($mode == 'url')
				{
					$value = $this->model_tool_seo_package->filter_seo($value, 'information', $row['information_id'], $lang);
	...
	public function generator_manufacturer($mode, $simulate) {
		...
		foreach ($rows as $row)
		{
			// todo : same functions for manufucter than prods, cats & infos
			...
			foreach ($types as $type)
			{
				...
				foreach ($rows as $row)
				{
					...
					$values['rows'][] = array(
						...
						'old_value' =>  '',
						'value' => str_replace(HTTP_SERVER, '/', $url),
						'changed' =>  0,
					);
	...
	public function install() {
		...
		{
			die('The automatic installation failed, please follow the steps for manual installation in the readme file.');
		}
