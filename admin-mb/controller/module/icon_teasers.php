...
class ControllerModuleiconteasers extends Controller {
	...
	public function index() {   
		$this->load->language('module/icon_teasers');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('icon_teasers', $this->request->post);		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/icon_teasers', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/icon_teasers', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		$modules = array();
		
		if (isset($this->request->post['icon_teasers_module'])) {
			$modules = $this->request->post['icon_teasers_module'];
		} elseif ($this->config->get('icon_teasers_module')) { 
			$modules = $this->config->get('icon_teasers_module');
		}
		...
		$this->template = 'module/icon_teasers.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		...
		if (!$this->user->hasPermission('modify', 'module/icon_teasers')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['icon_teasers_module'])) {
			foreach ($this->request->post['icon_teasers_module'] as $key => $value) {
				$module_number = $key;
