...
class ControllerModuleULTIMATUMAlsoBought extends Controller {
	...
	public function index() {   
		$this->load->language('module/ULTIMATUMalso_bought');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('ULTIMATUMalso_bought', $this->request->post);		
		...
		if (isset($this->error['dc_period'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/ULTIMATUMalso_bought', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/ULTIMATUMalso_bought', 'token=' . $this->session->data['token'], 'SSL');
		...
		foreach($languages as $language){
			$key = 'ULTIMATUMalso_bought_heading_title_' . $language['language_id'];
		...
		if (isset($this->request->post['ULTIMATUMalso_bought_dc_period'])){
			$this->data['ULTIMATUMalso_bought_dc_period'] = $this->request->post['ULTIMATUMalso_bought_dc_period'];
		} elseif ($this->config->get('ULTIMATUMalso_bought_dc_period')) {
			$this->data['ULTIMATUMalso_bought_dc_period'] = $this->config->get('ULTIMATUMalso_bought_dc_period');
		} else {
			$this->data['ULTIMATUMalso_bought_dc_period'] = 8;
		}
		
		
		$this->data['modules'] = array();
		
		if (isset($this->request->post['ULTIMATUMalso_bought_module'])) {
			$this->data['modules'] = $this->request->post['ULTIMATUMalso_bought_module'];
		} elseif ($this->config->get('ULTIMATUMalso_bought_module')) { 
			$this->data['modules'] = $this->config->get('ULTIMATUMalso_bought_module');
		}				
		...
		$this->data['orders_statuses'] = $this->model_module_ULTIMATUMalso_bought->getOrderStatuses();

		$this->template = 'module/ULTIMATUMalso_bought.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/ULTIMATUMalso_bought')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['ULTIMATUMalso_bought_module'])) {
			foreach ($this->request->post['ULTIMATUMalso_bought_module'] as $key => $value) {
		...
		if (!is_numeric($this->request->post['ULTIMATUMalso_bought_dc_period'])){
