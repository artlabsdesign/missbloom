...
class ModelCatalogOCFilter extends Model {
  ...
  public function copyAttributesToOCFilter($data = array()) {
    ...
    $this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value_to_product (product_id, option_id, value_id) SELECT product_id, attribute_id, SUBSTR(CRC32(CONCAT(attribute_id, text)), 1, 9) AS value_id FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value (option_id, value_id) SELECT attribute_id, SUBSTR(CRC32(CONCAT(attribute_id, text)), 1, 9) AS value_id FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY attribute_id, `text`");

		$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value_description (option_id, value_id, language_id, name) SELECT attribute_id, SUBSTR(CRC32(CONCAT(attribute_id, text)), 1, 9) AS value_id, language_id, text FROM " . DB_PREFIX . "product_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY attribute_id, `text`");
