...
class ModelModuleSeogen extends Model {
			...
			if($data['product_tag_table']) {
				...
				foreach($product_tags as $tag) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "product_tag`" .
						" SET `tag`='" . $this->db->escape($tag) . "', product_id='" . (int)$product['product_id'] . "', language_id='" . $language_id . "'");
				}
			} else {
	...
	private function generateManufacturer($manufacturer, $data) {
		...
		if(isset($manufacturer['seo_title']) && (isset($data['manufacturers_title_overwrite']) || (strlen(trim($manufacturer['seo_title']))) == 0)) {
			...
			$updates[] = "`seo_title`='" . $this->db->escape($manufacturers_title_template) . "'";
		}
	...
	private function getProducts($seogen, $product_id = false) {
		...
		$seogen['product_tag_table'] = $conf_seogen['product_tag_table'];
		...
		$query = $this->db->query("SELECT DISTINCT pd.*, m.name as 'manufacturer', p.model as 'model', p.sku, p.price, " .
								  ($seogen['product_tag_table'] ?
										  "(SELECT tag FROM `" . DB_PREFIX . "product_tag` pt WHERE pt.product_id = p.product_id AND pt.language_id = '" . $language_id . "' LIMIT 1) AS 'tag', " :
										  "pd.tag as 'tag', ") .
	...
	private function getManufacturers($manufacturer_id = false) {
		...
		$query = $this->db->query("SELECT md.*, u.keyword, m.name, m.manufacturer_id" .
