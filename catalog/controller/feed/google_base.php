<?php 
//			Новый класс. Выводит статьи из блога.
//
//
class ControllerFeedGoogleBase extends Controller {
	public function index() {
		if ($this->config->get('google_base_status')) { 
			...
			$output .= '<autor>'.$post['author_name'].'</autor>'; 
			$output .= '<category></category>'; 
			$output .= '<pubDate>'.$post['date_added'].'</pubDate>'; 
			$output .= '</item>'; 
			}
			...
			$this->response->setOutput($output, 1);
			//echo $output;
		}
	}
...
//			Старый класс. Выводит товары и категории
//
//
/* class ControllerFeedGoogleBase extends Controller {
	public function index() {
