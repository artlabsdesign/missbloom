...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		if(trim($url, '/') == 'buketi/belye-lilii-rozy.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/letnij-lug.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/lily/blagodarnost.html') {
			header('Location: http://missbloom.ru/buketi/lily', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/anglijskij-sad.html') {
			header('Location: http://missbloom.ru/buketi/lily', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/solnechnyj-vsplesk.html') {
			header('Location: http://missbloom.ru/buketi/lily', true, 301);
			exit();
		}
		if(trim($url, '/') == 'cveti/peonies-roses/pionovidnaya-roza-miranda.html') {
			header('Location: http://missbloom.ru/cveti/peonies-roses', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/sunflowers/solnechnyj-vsplesk.html') {
			header('Location: http://missbloom.ru/buketi/sunflowers', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/букеты-с-хризантемами/tsvetochnyj-lug.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/blagodarnost.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'cveti/peonies-roses/pionovidnaya-roza-o-hara.html') {
			header('Location: http://missbloom.ru/cveti/peonies-roses', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/rose/motsart.html') {
			header('Location: http://missbloom.ru/buketi/rose', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/rozovye-lilii-rozy.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/buket-hrizantemy-miks.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($_SERVER['REQUEST_URI'], '/') == 'buketi/anglijskij-sad.html?tag=белые розы') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/lily/motsart.html') {
			header('Location: http://missbloom.ru/buketi/lily', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/belaya-liliya.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/lily/romantika-krasnogo.html') {
			header('Location: http://missbloom.ru/buketi/lily', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/tsvetochnyj-lug.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/букеты-с-хризантемами/motsart.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'buketi/motsart.html') {
			header('Location: http://missbloom.ru/buketi', true, 301);
			exit();
		}
		if(trim($url, '/') == 'design-collection/tutti-frutti.html') {
			header('Location: http://missbloom.ru/design-collection', true, 301);
			exit();
		}
		...
		$this->load->model('setting/setting');
			...
			if (!empty($plus_minus_quantity)) {
				$this -> data['plus_minus_quantity'] = $plus_minus_quantity['plus_minus_quantity_module']['1']['enable_on_views']['product'];
		...
		if ($product_info) {
			...
			$NextProduct = $this->model_catalog_product->getProduct($product_id+1);
			$PrevProduct = $this->model_catalog_product->getProduct($product_id-1);
			$this->load->model('tool/image');
			
			if($NextProduct) {
				$this->data['next_product_href'] = $this->url->link('product/product', 'product_id=' . $NextProduct['product_id']);
				$this->data['next_product_name'] = utf8_substr(strip_tags(html_entity_decode($NextProduct['name'], ENT_QUOTES, 'UTF-8')), 0, 22) . '...';
				$this->data['next_product_thumb'] = $this->model_tool_image->resize($NextProduct['image'], 50, 50);
			} else {
				$this->data['next_product_href'] = '';
				$this->data['next_product_name'] = '';
				$this->data['next_product_thumb'] = '';
			}

			if($PrevProduct) {
				$this->data['prev_product_href'] = $this->url->link('product/product', 'product_id=' . $PrevProduct['product_id']);
				$this->data['prev_product_name'] = utf8_substr(strip_tags(html_entity_decode($PrevProduct['name'], ENT_QUOTES, 'UTF-8')), 0, 22) . '...';
				$this->data['prev_product_thumb'] = $this->model_tool_image->resize($PrevProduct['image'], 50, 50);
			} else {
				$this->data['prev_product_href'] = '';
				$this->data['prev_product_name'] = '';
				$this->data['prev_product_thumb'] = '';
			}
			...
			if ($product_info['image']) {
				...
				$this->data['thumbsmall'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'));
			} else {
				$this->data['thumb'] = '';
				$this->data['thumbsmall'] = '';
			}
			
			$manufacturer_image = $this->model_catalog_manufacturer->getManufacturer($product_info['manufacturer_id']);
         
            if($manufacturer_image){
               $this->data['brand_logo'] = $this->model_tool_image->resize($manufacturer_image['image'], 90, 90);
            }else{
               $this->data['brand_logo'] = false;
            }
