...
<?php if(($this->config->get('ULTIMATUMControl_cat_desc_status') == '1') && ($this->config->get('ULTIMATUMControl_status') == '1')){ ?> 
 <?php if (false && ($description || isset($seo_text))) { ?> 
...
<?php if(($this->config->get('ULTIMATUMControl_cat_refine_status') == '1') && ($this->config->get('ULTIMATUMControl_status') == '1') && !isset($_GET['filter_ocfilter'])){ ?>  
  <?php if ($categories) { ?>  
  ...
  <h2 class="heading_title"><span><?php echo $text_refine; ?></span></h2>
   ...
   <ul class="sub_cats">
          ...
          <li class="cat_hold" style="width:<?php echo ($this->config->get('ULTIMATUMControl_cat_refine_image_size_w')+ 10); ?>px; height:<?php echo ($this->config->get('ULTIMATUMControl_cat_refine_image_size_h') + 50); ?>px;"><a href="<?php echo $category['href']; ?>">
        <?php if(($this->config->get('ULTIMATUMControl_cat_refine_images') == '1') && ($this->config->get('ULTIMATUMControl_status') == '1')){ ?>
          <?php if ($category['pic']) { ?>
          <img src="<?php echo $category['pic']; ?>" title="<?php echo $category['name']; ?>" alt="<?php echo $category['name']; ?>" />
          <?php } else { ?>
          <img src="<?php echo $no_cat_pic; ?>" width="<?php echo $this->config->get('ULTIMATUMControl_cat_refine_image_size_w'); ?>" height="<?php echo $this->config->get('ULTIMATUMControl_cat_refine_image_size_h'); ?>" title="<?php echo $category['name']; ?>" alt="<?php echo $category['name']; ?>" />
        <?php } ?>  
          <?php } ?>
          <span>
...
<?php } ?>
  ...
  <a href="<?php echo $compare; ?>" id="compare-total"><i class="icon-bar-chart"></i> <?php echo $text_compare; ?></a> 
        ...
        ?>
            ...
            <div class="price">
              ...
              <div class="count_holder_small">
                ...
                <div id="ULTIMATUMCountSmallcategory<?php echo $product['product_id']; ?>"></div>
                <script type="text/javascript">
					$(function () {	
						$('#ULTIMATUMCountSmallcategory<?php echo $product['product_id']; ?>').countdown({until: new Date(<?php echo $product['date_end']; ?>), compact: false, layout: '<span class="countdown_time_small">{dn} {dl} {hn}:{mnn}:{snn}</span>'});
					});
		</script>
                <div class="clear"></div>
              </div>
            ...
            <?php } ?>
            <div class="cart"> <a class="add_to_cart_small" title="<?php echo $button_cart; ?>" onclick="<?php if (($this->config->get('ULTIMATUMControl_status') == '1') && ($this->config->get('ULTIMATUMControl_countdown_sold') == '1') && ($product['quantity'] < '1')) { ?><?php } else { ?>addToCart('<?php echo $product['product_id']; ?>'<?php echo $product['minimum']>1?',\''.$product['minimum'].'\'':''; ?>);<?php } ?>"><i class="icon-shopping-cart"></i> <?php echo $button_cart; ?></a> </div>
...
<?php } ?>  
  ...
  <?php if(($this->config->get('ULTIMATUMControl_cat_desc_status') == '1') && ($this->config->get('ULTIMATUMControl_status') == '1')){ ?> 
	<?php if ($description || isset($seo_text)) { ?> 
	<div class="category-info" style="padding: 0;">
	<?php if (isset($seo_text)) { 
	echo $seo_text;
	}else if($description){
	echo $description;  
