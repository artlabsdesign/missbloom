...
function checkout() {
    ...
    $.ajax({
        type: 'POST',
        url: 'index.php?route=checkout/ajaxcheckout/simpleorder',
        dataType: 'json',
        ...
        success: function(json) {
            if (json['error']){
                  $("#gotoorderajax").bind("click", function () {
                    $("#cboxContent").mask("<?php echo $text_ajaxcheckout_loading; ?>");
                  });
                ...
                if (json['output']) {
                  $("#cboxContent").unmask();
                  $('#ajaxordermainbody').html('<div id="ajaxcheckoutsuccess">'+json['output']+'</div>');
                  $(this).colorbox.resize();
                }
            }
        }
...
function pluscon(pid) {
  ...
  $("#quant-"+pid).val(qua);
  $.ajax({
    url: 'index.php?route=checkout/ajaxcheckout/calc&product_id='+pid+'&qty='+qua,
    type: 'post',
    dataType: 'json',
...
function ups() { 
  $.ajax({
    url: 'index.php?route=checkout/ajaxcheckout/calc',
    type: 'post',
    dataType: 'json',
    data: $('#ajaxorder input[type=\'text\'], #ajaxorder  input[type=\'hidden\'], #ajaxorder  input[type=\'radio\']:checked, #ajaxorder  input[type=\'checkbox\']:checked, #ajaxorder  select, #ajaxorder textarea'),
    success:function(json) {
...
<div id="ajaxorder">
  ...
  <div id="ajaxcheckout"><?php echo $text_ajaxcheckout_head; ?></div>
  <?php if ($hideimg == 1) { ?>
    <div id="ajaxorderimg">
        ...
        <div class="image-additional">
          <div class="carousel-checkout"> 
            <div class="carousel-button-left-checkout"><a href="javascript:void(0);"><img src="image/ajaxcheckout/carousel-left.png"/></a></div> 
            <div class="carousel-button-right-checkout"><a href="javascript:void(0);"><img src="image/ajaxcheckout/carousel-right.png"/></a></div> 
  ...
  <?php } ?>
    ...
    <table id="aboutproduct">
      <thead>
        <tr>
          <td style="width:40%;"><span class="aboutproductspan"><?php echo $text_ajaxcheckout_ibuy; ?></span></td>
          <td style="width:20%;"><span class="aboutproductspan"><?php echo $text_ajaxcheckout_priceof; ?></span></td>
          <td style="width:20%;text-align:center;"><?php echo $text_ajaxcheckout_inanamount; ?></td>
        </tr>
      </thead>
      <tbody>
        <tr>
    ...
    <br />
      <?php if ($hidefio == 1) { ?>
        ...
        <div class="sections_block_rquaired">
          ...
          <input type="text" name="firstname" class="ajaxorderinputsnorequired" placeholder="<?php echo $text_ajaxcheckout_entername; ?>"/>
          <div id="error_firstname" class="ajaxcheckout_errorfields"></div>
        </div>
        <?php } else { ?>
        <div class="sections_block">
          <i class="icon-append_1 icon-user"></i>
          <input type="text" name="firstname" class="ajaxorderinputs" placeholder="<?php echo $text_ajaxcheckout_entername; ?>"/>
        </div>
        <?php } ?>
      <?php } ?>
      <?php if ($hideemail == 1) { ?>
        ...
        <div class="sections_block_rquaired">
          ...
          <input type="text" name="email" class="ajaxorderinputsnorequired" placeholder="<?php echo $text_ajaxcheckout_enteremail; ?>"/>
          <div id="error_email" class="ajaxcheckout_errorfields"></div>
        </div>
        <?php } else { ?>
        <div class="sections_block">
          <i class="icon-append_1 icon-envelope-alt"></i>
          <input type="text" name="email" class="ajaxorderinputs" placeholder="<?php echo $text_ajaxcheckout_enteremail; ?>"/>
        </div>
        <?php } ?>
      <?php } ?>
      <?php if ($hidetelephone == 1) { ?>
        ...
        <div class="sections_block_rquaired">
          ...
          <input type="text" name="telephone" class="ajaxorderinputsnorequired" id="fortelephonemask" onclick="$(this).inputmask('<?php echo $mask_telephone; ?>');" placeholder="<?php echo $text_ajaxcheckout_entertelephone; ?>"/>
          <div id="error_telephone" class="ajaxcheckout_errorfields"></div>
        </div>
        <?php } else { ?>
        <div class="sections_block">
          <i class="icon-append_1 icon-phone"></i>
          <input type="text" name="telephone" class="ajaxorderinputs" id="fortelephonemask" onclick="$(this).inputmask('<?php echo $mask_telephone; ?>');" placeholder="<?php echo $text_ajaxcheckout_entertelephone; ?>"/>
        </div>
        <?php } ?>
      <?php } ?>
      <?php if ($hidedescription == 1) { ?>
          <textarea name="description" class="ajaxorderinputs" placeholder="<?php echo $text_ajaxcheckout_enterdescription; ?>"></textarea>
      <?php } ?>
        <input name="product_id" value="<?php echo $product_id ?>" type="hidden" />
      <?php if ($hidepayment == 1) { ?> 
        <div class="pay_ship_block">
          <div class="fieldname"><?php echo $text_ajaxcheckout_selectpayment; ?>: <?php if($config_info_payment == 1) { ?><a class="faqinfo" faq="<?php echo $info_payment_text; ?>">[?]</a><?php } ?></div>
      ...
      <?php if ($hideshipping == 1) { ?> 
        <div class="pay_ship_block">
          <div class="fieldname"><?php echo $text_ajaxcheckout_selectshipping; ?>: <?php if($config_info_shipping == 1) { ?><a class="faqinfo" faq="<?php echo $info_shipping_text; ?>">[?]</a><?php } ?></div>
              ...
              <?php if ($option['type'] == 'select') { ?>
                <div id="option-<?php echo $option['product_option_id']; ?>" class="option">
                  ...
                  <select onchange="ups();"  name="option[<?php echo $option['product_option_id']; ?>]" >
                    <option value=""><?php echo $text_ajaxcheckout_selectoption; ?></option>
    ...
    <a onclick="$.colorbox.close();" id="cancelorderajax"><?php echo $text_ajaxcheckout_cancel; ?></a>
    <input type="button" onclick="checkout();" id="gotoorderajax" value="<?php echo $text_ajaxcheckout_ordernow; ?>">
  </div>
</div>